...
def gm_freq_offset(lat_cp, lon_cp, ht_cp, ray_bearing, freq, plas_freq, UT):
    ...
    if np.ndim(lat_cp) != 0 or np.ndim(lon_cp) != 0 or np.ndim(ht_cp) != 0 or \
       np.ndim(ray_bearing) != 0 or np.ndim(freq) != 0 or \
       np.ndim(plas_freq) != 0:
...
def h_param(W, theta, OX_mode):
    ...
    #M the equation. 
    scalar_input = np.ndim(theta) == 0 and np.ndim(W) == 0
    theta = np.array(theta, dtype=float, copy=True, ndmin=1)
    W = np.asarray(W, dtype=float)
    
    #M stay away from theta = 90 deg as denominator -> 0 here for X mode
    idx = np.where((theta > np.pi / 2 - 1e-5) & (theta < np.pi / 2 + 1e-5))
    theta[idx] = theta[idx] - 1e-5
    
    S2 = np.sin(theta) ** 2
    C2 = np.cos(theta) ** 2
    
    fact = np.sqrt(1 + 4 * C2 * W ** 2 / S2 ** 2)
