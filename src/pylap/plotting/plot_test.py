...
def plot_test(iono_grid, start_range, end_range, range_inc,
    ...
    # print(start_height,end_height, height_inc, no_heights, iono_grid_size[0])
    # print(heights)
    ...
    image = plt.pcolormesh(iono_X, iono_Y, iono_grid,shading='flat')
    ax.set_aspect('equal')
    ax.axis('off')  # turn off rectangler suround box and tic marks
    ...
    # if ((num_ticks - 1) .* tick_stepsize < end_height - tick_stepsize)
       ...
       # if pp < length(acceptable_tick_stepsize)
            if pp < (acceptable_tick_stepsize):
